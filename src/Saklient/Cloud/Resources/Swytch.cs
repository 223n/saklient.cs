...
namespace Saklient.Cloud.Resources
{
    ...
    public class Swytch : Resource
    {
        ...
        public override string _ClassName() => "Swytch";
        ...
        /// </summary>
        public Swytch Save() => ((Swytch)(this._Save()));

        /// <summary>最新のリソース情報を再取得します。
        /// 
        /// <returns>this</returns>
        /// </summary>
        public Swytch Reload() => ((Swytch)(this._Reload()));
        ...
        internal object _UsedIpv4AddressHash()
        {
            ...
            for (int __it1 = 0; __it1 < (ifaces as System.Collections.IList).Count; __it1++)
            {
                ...
                if (ip == null) ip = userIp;
                if (ip != null) (found as System.Collections.Generic.Dictionary<string, object>)[ip] = true;
            }
            return found;
        }
        ...
        private string Get_userDefaultRoute() => this.M_userDefaultRoute;

        /// <summary>ユーザ設定IPv4ネットワークのゲートウェイ
        /// </summary>
        public string UserDefaultRoute => this.Get_userDefaultRoute();

        private bool N_userMaskLen = false;

        private long? Get_userMaskLen() => this.M_userMaskLen;

        /// <summary>ユーザ設定IPv4ネットワークのマスク長
        /// </summary>
        public long? UserMaskLen => this.Get_userMaskLen();

        private bool N_router = false;

        private Router Get_router() => this.M_router;

        /// <summary>接続されているルータ
        /// </summary>
        public Router Router => this.Get_router();

        private bool N_bridge = false;

        private Bridge Get_bridge() => this.M_bridge;

        /// <summary>接続されているブリッジ
        /// </summary>
        public Bridge Bridge => this.Get_bridge();

        private bool N_ipv4Nets = false;

        private System.Collections.Generic.List<Ipv4Net> Get_ipv4Nets() => this.M_ipv4Nets;

        /// <summary>IPv4ネットワーク（ルータによる自動割当） <see cref="Saklient.Cloud.Resources.Ipv4Net" /> の配列
        /// </summary>
        public System.Collections.Generic.List<Ipv4Net> Ipv4Nets => this.Get_ipv4Nets();

        private bool N_ipv6Nets = false;

        private System.Collections.Generic.List<Ipv6Net> Get_ipv6Nets() => this.M_ipv6Nets;

        /// <summary>IPv6ネットワーク（ルータによる自動割当） <see cref="Saklient.Cloud.Resources.Ipv6Net" /> の配列
        /// </summary>
        public System.Collections.Generic.List<Ipv6Net> Ipv6Nets => this.Get_ipv6Nets();
