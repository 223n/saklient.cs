...
namespace Saklient.Cloud.Resources
{
    ...
    public class ServerInstance : Resource
    {
        ...
        /// </summary>
        public bool IsUp() => this.Get_status() != null && EServerInstanceStatus.Compare(this.Get_status(), EServerInstanceStatus.UP) == 0;

        /// <summary>サーバが停止しているときtrueを返します。
        /// </summary>
        public bool IsDown() => this.Get_status() == null || EServerInstanceStatus.Compare(this.Get_status(), EServerInstanceStatus.DOWN) == 0;
        ...
        private string Get_beforeStatus() => this.M_beforeStatus;

        /// <summary>前回の起動状態 <see cref="Saklient.Cloud.Enums.EServerInstanceStatus" />
        /// </summary>
        public string BeforeStatus => this.Get_beforeStatus();

        private bool N_statusChangedAt = false;

        private System.DateTime? Get_statusChangedAt() => this.M_statusChangedAt;

        /// <summary>現在の起動状態に変化した日時
        /// </summary>
        public System.DateTime? StatusChangedAt => this.Get_statusChangedAt();

        private bool N_isoImage = false;

        private IsoImage Get_isoImage() => this.M_isoImage;

        /// <summary>挿入されているISOイメージ
        /// </summary>
        public IsoImage IsoImage => this.Get_isoImage();
