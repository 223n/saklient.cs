...
namespace Saklient
{
    ...
    public class Util
    {
        ...
        public static object DecodeJson(string json) => ArrayList2List(serializer.Deserialize<Hash>(json));

        public static string EncodeJson(object obj) => serializer.Serialize(obj);
        ...
        public static bool ExistsPath(object obj, string path)
        {
            ...
            foreach (string key in aPath)
            {
                ...
                if (Regex.IsMatch(key, @"^\d+$"))
                {
                    ...
                    if (!(obj is List<object> arr) || arr.Count <= idx) return false;
                    obj = arr[idx];
                }
                else
                {
                    if (!(obj is Hash dic) || !dic.ContainsKey(key)) return false;
                    obj = dic[key];
                }
            }
            return true;
        ...
        public static object GetByPath(object obj, string path)
        {
            ...
            foreach (string key in aPath)
            {
                ...
                {
                    int idx = Convert.ToInt32(key);
                    if (!(obj is List<object> arr) || arr.Count <= idx) return null;
                    obj = arr[idx];
                }
                else
                {
                    if (!(obj is Hash dic) || !dic.ContainsKey(key)) return null;
                    obj = dic[key];
                }
            }
            return obj;
        ...
        public static DateTime? Str2date(string s) => (DateTime?)DateTime.Parse(s);

        public static string Date2str(DateTime? d) => d == null ? null : ((DateTime)d).ToString("s");

        public static long Ip2long(string str)
        {
            if (!Regex.IsMatch(str, @"^\d+\.\d+\.\d+\.\d+$")) return -1;
            if (!IPAddress.TryParse(str, out IPAddress ip)) return -1;
        ...
        {
            if (!IPAddress.TryParse(((UInt32)(long)val).ToString(), out IPAddress ip)) return null;
            return ip.ToString();
        }
        ...
        public static void Sleep(long sec) => Thread.Sleep(System.Convert.ToInt32(1000 * sec));
